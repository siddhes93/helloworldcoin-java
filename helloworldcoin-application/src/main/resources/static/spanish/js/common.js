...
function transactionHtml(item){
    ...
    if(!isNull(transactionInputs)){
        transactionInputs.forEach(item1 => {
            left += `<div>pagador：<span><a title="Detalles de la dirección" target="_blank" href="./address.html?address=${item1.address}">${item1.address}</a></span>&nbsp;<a title="Detalles de la salida de la transacción" target="_blank" href="./transactionoutput.html?transactionHash=${item1.transactionHash}&transactionOutputIndex=${item1.transactionOutputIndex}"><i class="glyphicon-euro"></i></a>&nbsp;<span>${item1.value}</span></div>`
        })
    }
    ...
    if(!isNull(transactionOutputs)){
        transactionOutputs.forEach(item1 => {
            right += `<div style="display:flex">beneficiario：<span><a title="Detalles de la dirección" target="_blank" href="./address.html?address=${item1.address}">${item1.address}</a></span>&nbsp;<a title="Detalles de la salida de la transacción" target="_blank" href="./transactionoutput.html?transactionHash=${item1.transactionHash}&transactionOutputIndex=${item1.transactionOutputIndex}"><i class="glyphicon-euro"></i></a>&nbsp;<span>${item1.value}</span></div>`
        });
    }

    let itemTransactionType = item.transactionType == 'GENESIS_TRANSACTION'?"Transacción de Génesis":"Transacción estándar";
    ...
    let transactionHtml = `
        <div style="font-size: 14px; line-height: 40px; margin-top: 10px;">
            ...
            <div style="display: flex; background-color: #f5f5f5; flex-wrap: wrap; border-top: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                <div style="min-width: 40%;"><a title="Detalles de la transacción" target="_blank" href="./transaction.html?transactionHash=${item.transactionHash}">${item.transactionHash}</a></div>
