...
function transactionHtml(item){
    ...
    if(!isNull(transactionInputs)){
        transactionInputs.forEach(item1 => {
            left += `<div>payeur：<span><a title="Détails de l'adresse" target="_blank" href="./address.html?address=${item1.address}">${item1.address}</a></span>&nbsp;<a title="Détails de la sortie de la transaction" target="_blank" href="./transactionoutput.html?transactionHash=${item1.transactionHash}&transactionOutputIndex=${item1.transactionOutputIndex}"><i class="glyphicon-euro"></i></a>&nbsp;<span>${item1.value}</span></div>`
        })
    }
    ...
    if(!isNull(transactionOutputs)){
        transactionOutputs.forEach(item1 => {
            right += `<div style="display:flex">bénéficiaire：<span><a title="Détails de l'adresse" target="_blank" href="./address.html?address=${item1.address}">${item1.address}</a></span>&nbsp;<a title="Détails de la sortie de la transaction" target="_blank" href="./transactionoutput.html?transactionHash=${item1.transactionHash}&transactionOutputIndex=${item1.transactionOutputIndex}"><i class="glyphicon-euro"></i></a>&nbsp;<span>${item1.value}</span></div>`
        });
    }

    let itemTransactionType = item.transactionType == 'GENESIS_TRANSACTION'?"Transaction Genesis":"Transaction standard";
    ...
    let transactionHtml = `
        <div style="font-size: 14px; line-height: 40px; margin-top: 10px;">
            ...
            <div style="display: flex; background-color: #f5f5f5; flex-wrap: wrap; border-top: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                <div style="min-width: 40%;"><a title="Détails de la transaction" target="_blank" href="./transaction.html?transactionHash=${item.transactionHash}">${item.transactionHash}</a></div>
